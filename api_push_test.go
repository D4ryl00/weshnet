...
type serviceMethods interface {
	GetContextGroupForID(id []byte) (*weshnet.GroupContext, error)
	GetCurrentDevicePushConfig() (*protocoltypes.PushServiceReceiver, *protocoltypes.PushServer)
}

func createVirtualOtherPeerSecretsShareSecret(t testing.TB, ctx context.Context, membersStores []*weshnet.MetadataStore) (*cryptoutil.OwnMemberDevice, *protocoltypes.DeviceSecret) {
	...
	for _, store := range membersStores {
		_, err = weshnet.MetadataStoreAddDeviceToGroup(ctx, store, store.Group(), otherMD)
		require.NoError(t, err)

		memPK, err := store.MemberPK()
		require.NoError(t, err)

		_, err = weshnet.MetadataStoreSendSecret(ctx, store, store.Group(), otherMD, memPK, ds)
		require.NoError(t, err)
	}
...
func Test_sealPushMessage_decryptOutOfStoreMessageEnv(t *testing.T) {
	...
	otherMD, otherDS := createVirtualOtherPeerSecretsShareSecret(t, ctx, []*weshnet.MetadataStore{gc.MetadataStore()})
