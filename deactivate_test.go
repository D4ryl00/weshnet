...
import (
	...
	"berty.tech/weshnet/internal/ipfsutil"
...
func TestReactivateAccountGroup(t *testing.T) {
	...
	nodeC, closeNodeC := NewTestingProtocol(ctx, t, &TestingOpts{
	...
	nodes := []*TestingProtocol{nodeA, nodeB}
	...
	nodes = []*TestingProtocol{nodeA, nodeC}
...
func TestReactivateContactGroup(t *testing.T) {
	...
	opts := TestingOpts{
		...
		Logger:      logger,
		ConnectFunc: ConnectAll,
	}

	nodes, cleanup := NewTestingProtocolWithMockedPeers(ctx, t, &opts, nil, 2)
	defer cleanup()
